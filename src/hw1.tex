...
\begin{itemize}
\item Suppose $A_{j,k-1}[k-1]>A_j[j]$. Then the inner loop body executes. By the induction hypothesis, the invariant holds for $i=k$. The only modification is that the value $A_j[k-1]$ is assigned into $A_{j,k-1}[k]$.
\begin{enumerate}[nosep]
    \item The first $k-1$ elements remain unchanged. Thus, on iteration $i=k-1$, we have $A_{j,k-1}[1,\dots,k-1]=A_j[1,\dots,k-1]$.
    \item The elements between indices $k+1$ and $j$ also remain unchanged. Thus, if $k+1\leq j$, then $A_j[k+1,\dots,j]=A_{j,k-1}[k,\dots,j-1]$. Since the invariant holds for $i=k$, we know that $A_j[j]<A_{j,k}[k+2]$. Since the value $A_j[k-1]$ was greater than $A_j[j]$ and was shifted into $A_{j,k-1}[k]$, we know $A_j[j]<A_{j,k-1}[k+1]$. Before the inductive step, $A_j[j]$ was smaller than the first unshifted element $A_{j,k}[k+2]$. After the inductive step, $A_j[j]$ remains smaller than the first unshifted element $A_{j,k-1}[k+1]$.
\end{enumerate}
The loop invariant holds in this case.
\item Suppose instead that $A_{j,k-1}[k-1]\leq A_j[j]$. Then the inner loop body does not execute. Thus $A_{j,k-1}=A_{j,k}$. The loop invariant holds in this case.
\end{itemize}
...
\textit{Inductive step. }Consider $j=m+1$. By the induction hypothesis, we have $A_m[1,\dots,m-1]$ contains the elements $A[1,\dots,m-1]$ in sorted order. The inner loop invariant asserts that $A_{m+1}[\ell]=A[\ell]$ for $m+1\leq\ell\leq n$. The loop body assigns $A[m+1]$ into a position such that, by the inner loop invariant, $A[m]\leq A[m+1]\leq A[m+2]$. Thus, $A_{m+1}[1,\dots,m]$ contains the elements $A[1,\dots,m]$ in sorted order.  The loop invariant holds.
\end{solution}
\end{enumerate}
