...
\begin{itemize}
    \item Suppose $(T[k],T[k+1])\in E$. Then there exists a path from $T[1]$ to $T[k+1]$, noting $T[1]$ precedes $T[k]$ and $T[k]$ precedes $T[k+1]$ in the topological sort. Thus, Property 1 holds for the induced subgraph containing only the vertices in $T[1,\dots,k]$.
